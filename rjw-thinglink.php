<?php
	...
	*/
	...
	class RJW_Thinglink {
		...
		function add_thinklink_to_footer(){
		  ...
		  echo "
				<script type=\"text/javascript\">
					__tlid = " . wp_json_encode( esc_html( $thinglink_id ) ) . ";
		...
		function admin_menu(){
			add_submenu_page( 'plugins.php', self::PLUGIN_NAME, self::PLUGIN_NAME, 'manage_options', self::PLUGIN_CONFIG_HOOK, array(
				&$this, 
				'admin_menu_options',
			) );
		}
		...
		function plugin_action_links( $links, $file ) {
			if ( $file == "rjw-thinglink/rjw-thinglink.php" ) {
				...
				$text = __( 'Settings' );
				array_unshift( $links, '<a href="' . esc_url( $href ) . '">' . esc_html( $text ) . '</a>' );
			}
			return $links;
		}
